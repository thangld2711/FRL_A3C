...
sys.path.append(config.PROJECT_PATH + "/frl_classess/")
import msf_wrapper
import fl_a3c_classes
import scan_cluster
import queue
import service_scan
...
TOTAL_EPS = 10  # Number of episodes
...
class MasterAgent():
    def __init__(self, host, name):
        ...
        if self.name == "main":
            ...
            try:
                model_path = os.path.join(self.save_dir, 'pwn_model_main.h5')
    ...
    def train(self, result):
        ...
        self.global_model.save_weights(
            os.path.join(self.save_dir,
                         f'pwn_model_client_{self.name}.h5')
        )
        ...
        while global_episode < TOTAL_EPS:
            for port in self.ports:
                ...
                if reward > 0:
                    ...
                    # global_avg_reward.append(global_moving_average_reward)
                    # global_avg_loss.append(self.ep_loss)
                ep_steps += 1
...
class FederatedLearning:
    def __init__(self, hosts, scan_results):
        self.scan_results = scan_results
        self.hosts = hosts
        self.main_agent = MasterAgent(self.hosts, 'main')
        self.create_client()
        self.number_of_client = 3
        self.rounds = 3
    ...
    def create_client(self):
        ...
        for i in range(3):
            ...
            self.clients[f"client_{i}"] = MasterAgent(self.hosts, i)
            self.updated_clients[f"client_{i}"] = 0 

    def update_client(self):
        with tf.GradientTape() as tape:
            def update(client_model, main_model):
                weights = main_model.get_weights()
                client_model.set_weights(weights)
            for i in range(3):
                update(self.clients[self.client_names[i]].global_model, self.main_agent.global_model)

    def update_main_agent(self):
        # list_of_weights = []
        # for i in range(3):
        local_weights = [self.clients[self.client_names[i]].global_model.get_weights() for i in range(3)]
        # avg_weights = tf.reduce_mean(list_of_weights, axis = 0)
        # self.main_agent.global_model.set_weights(avg_weights)
        new_weights = []
        for main_param, local_params in zip(self.main_agent.global_model.trainable_weights, zip(*local_weights)):
            weighted_avg = tf.zeros_like(local_params[0])
            for local_param in local_params:
                weighted_avg += local_param
            weighted_avg /= 3  # Chia tổng trọng số cho số lượng mạng mô hình local
            # print(weighted_avg)
            new_weights.append(weighted_avg.numpy())
            # print(new_weights)
        self.main_agent.global_model.set_weights(new_weights)

    def step(self, idx_user, round_no):
        self.update_client()
        for user in idx_user:
            print(f"Client {user}")
            avg_reward = self.clients[self.client_names[user]].train(self.scan_results)

            print(f"LOCAL TRAIN: ------------------------------------------------------")
            print(f"AVG_REWARD {len(avg_reward)}")
        
        self.update_main_agent()

    def run(self):
        m = 2
        for round_no in range(self.rounds):
            idx_users = np.random.choice(range(3), m, replace=False)

            for user in idx_users:
                self.updated_clients[f"client_{user}"] = round_no + 1
            
            self.step(idx_users, round_no)
            print(f"==============update-round-{round_no}=================")

        self.main_agent.global_model.save_weights(
            os.path.join(self.main_agent.save_dir,
                         'pwn_model_main.h5')
        )
